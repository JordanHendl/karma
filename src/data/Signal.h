 ...
 * Purpose      : Declaration for Data Signal object.
**********************************************************************/
...
namespace data
{
  namespace module
  {
    ...
    /** Global recursive compile-time function to implement a crc32 string hash.
     * @tparam idx The index of hash the function is currently at.
     * @param str The string to hash
     * @return constexpr unsigned The numerical representation of this string.
     */
    template<FunctionIdentifier idx>
    constexpr FunctionIdentifier crc32( const char * str ) ;

    /** The stop recursion function to the compile-time string hashing algorithm.
     * @tparam  None
     */
    template<>
    constexpr FunctionIdentifier crc32<FunctionIdentifier( -1 )>( const char * str ) ;

    /** Wrapper function for the hashing algorithm.
     */
    template<typename T>
    ...
    void Signal::attach( void (*method)( T ) )
    {
      ...
      attachBase( hash<T>(), dynamic_cast<Callback*>( new CallbackMethod( method ) ) ) ;
    }
